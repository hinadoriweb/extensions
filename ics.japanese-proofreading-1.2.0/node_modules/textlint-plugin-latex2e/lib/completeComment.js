...
const insertComment = (comment, nodes) => {
    if (!nodes) {
        return (0, exports.insertComment)(comment, []);
    }
    for (let i = 0; i < nodes.length; i++) {
        // If the comment is appeared before the node, insert it before the node.
        if ((0, exports.isAppearedBeforeNode)(nodes[i].range, comment.range)) {
            nodes.splice(i, 0, comment);
            return nodes;
        }
        // If the comment is included in the node, try to insert it recursively.
        if ((0, exports.isIncludedByNode)(nodes[i].range, comment.range)) {
            if ((0, exports.isParentNode)(nodes[i])) {
                nodes[i].children = (0, exports.insertComment)(comment, nodes[i].children);
                return nodes;
            }
            switch (nodes[i].type) {
...
const completeComments = (comments) => (rawText) => (root) => {
    ...
    const textlintComments = (0, exports.convertCommentToTxtNode)(rawText, comments);
    const copiedRoot = JSON.parse(JSON.stringify(root));
    for (const comment of textlintComments) {
        copiedRoot.children = (0, exports.insertComment)(comment, copiedRoot.children);
    }
    return copiedRoot;
};
